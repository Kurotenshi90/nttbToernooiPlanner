 ...
 */
public class SpelersInschrijvenController implements Initializable {
    private int toernooiID;
    private SpelersInschrijvenModel spelersInschrijvenModel;
    ...
    @FXML ComboBox<Deeltoernooi> Deeltoernooi;
    ...
    public SpelersInschrijvenController(int toernooiID){
        this.toernooiID = toernooiID;
    }

    @Override
    public void initialize(URL location, ResourceBundle resources) {
        spelersInschrijvenModel = new SpelersInschrijvenModel();
        spelersInschrijvenModel.getAllVerenigings();
        spelersInschrijvenModel.getToernooi(toernooiID);
        setPartnerInvis();
    ...
    private void initializeButtons() {
        DeelnemerToevoegen.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                if (Deeltoernooi.getSelectionModel().getSelectedItem().getSpelvorm().equals("Dubbel")){
                    ...
                    spelersInschrijvenModel.addDeelnemer(partner);
                }
                spelersInschrijvenModel.addDeelnemer(deelnemer);
        ...
        Opslaan.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent event) {
                spelersInschrijvenModel.saveDeelnemers();
                goToHome(Opslaan);
            }
        });
    ...
    private void initializeDeeltoernooi() {
        Deeltoernooi.getItems().setAll(spelersInschrijvenModel.getToernooi().getDeeltoernoois());
    }

    private void initializeVereniging() {
        Vereniging.getItems().setAll(spelersInschrijvenModel.getVerenigings());
    }
    ...
    private void initializeToernooiGegevens() {
        ...
        Toernooinaam.setText(spelersInschrijvenModel.getToernooi().getNaam());
        Spelvorm.setEditable(false);
        Spelvorm.setText(spelersInschrijvenModel.getToernooi().getToernooisoort());
        Begindatum.setEditable(false);
        Begindatum.setText(spelersInschrijvenModel.getToernooi().getBegindatum().toString());
        Inschrijfdatum.setEditable(false);
        Inschrijfdatum.setText(spelersInschrijvenModel.getToernooi().getInschrijfdatum().toString());
        Einddatum.setEditable(false);
        Einddatum.setText(spelersInschrijvenModel.getToernooi().getEinddatum().toString());
    }
